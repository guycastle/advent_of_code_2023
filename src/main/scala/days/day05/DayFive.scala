...
object DayFive extends DailyChallenge[Long]:
  ...
  lazy val seedRangeParser: String => (Seq[SeedType], Long) = str =>
    val ranges = stringToLongs(str).sliding(2, 2).toSeq.map {
      case Seq(start, length) => (start until (start + length))
    }.sortBy(_.start).foldLeft(Seq.empty[NumericRange[Long]]) {
  ...
  def almanacParser(input: Seq[String], seedParser: String => (Seq[SeedType], Long)): Option[Almanac] =
    input.collectFirst {
      case s"seeds: $numbers" => seedParser(numbers)
    }.map( (seeds, total) => {
    ...
    (for
      ...
      closest <- almanac.seeds.foldLeft(Option.empty[Long]) {
        ...
        case (closest, range: NumericRange[Long]) =>
          range.foldLeft(closest) {
            case (rangeClosest, seed) =>
              println(s"Evaluating seed $seed (${pctFmt.format(counter.incrementAndGet/almanac.totalSeeds)})")
              val seedLocation = almanac.locationForSeed(seed)
  ...
  case class Almanac(seeds: Seq[SeedType],
    ...
    def locationForSeed(seed: Long): Long =
      this.humidityToLocations.get(this.temperaturesToHumidity.get(this.lightsToTemperatures.get(this.watersToLights.get(this.fertilizersToWaters.get(this.soilsToFertilizers.get(this.seedsToSoils.get(seed)))))))
  end Almanac
