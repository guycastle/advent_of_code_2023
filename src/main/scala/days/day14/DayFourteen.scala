...
object DayFourteen extends DailyChallenge[Int]:
  ...
  extension (array: DishArray)
    def remove(tile: Tile):               DishArray = array.updated(
      tile.coords.y,
      array.lift(tile.coords.y).getOrElse(Map.empty).removed(tile.coords.x)
    )
  ...
  lazy val parseInput: Seq[String] => DishArray = lines =>
    lines.zipWithIndex.map: (line, y) =>
      line.zipWithIndex.foldLeft(Map.empty[Int, Tile]):
  ...
  def moveRoundedRocksUp(array: DishArray): DishArray = array
    .drop(1)
    .foldLeft(array):
