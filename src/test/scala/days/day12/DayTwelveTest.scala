...
class DayTwelveTest extends BaseTest:
  ...
  "Day twelve's solution" must:
    ...
    "parse the input correctly" in:
      test.headOption.map(DayTwelve.parseInputLine).value mustBe Record(statuses = Seq(Unknown, Unknown, Unknown, Operational, Damaged, Damaged, Damaged), contiguousDamaged = Seq(1, 1, 3))
